...
def constructBayesNet(gameState: hunters.GameState):
    ...
    max_manhattan = (X_RANGE - 1) + (Y_RANGE - 1)
    observation_values = list(range(max_manhattan + MAX_NOISE + 1))
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            involvedFactors = [f for f in factors if var in f.variablesSet()]
            factors = [f for f in factors if var not in f.variablesSet()]

            if not involvedFactors:
                continue

            _, joinedFactor = joinFactorsByVariable(involvedFactors, var)
            if len(joinedFactor.unconditionedVariables()) > 1:
                    reducedFactor = eliminate(joinedFactor, var)
                    factors.append(reducedFactor)
            
        
        if not factors:
            return None
        resultFactor = factors[0]
        for f in factors[1:]:
            resultFactor = joinFactors([resultFactor, f])

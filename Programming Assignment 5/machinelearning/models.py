...
class PerceptronModel(Module):
    ...
    def run(self, x):
        ...
        """
        "*** YOUR CODE HERE ***"
        return torch.sum(self.w * x, dim=1, keepdim=True)
    ...
    def get_prediction(self, x):
        ...
        "*** YOUR CODE HERE ***"
        score = self.run(x)
        return torch.where(score >= 0, torch.tensor(1), torch.tensor(-1))
...
class LanguageIDModel(Module):
    ...
    def run(self, xs):
        ...
        """
        "*** YOUR CODE HERE ***"
        h = torch.tanh(self.W_x(xs[0]))

        for x in xs[1:]:
            z = torch.tanh(self.W_x(x) + self.W_hidden(h))
            h = torch.relu(z)
    ...
    def get_loss(self, xs, y):
        ...
        "*** YOUR CODE HERE ***"
        logits = self.run(xs)
        return cross_entropy(logits, torch.argmax(y, dim=1))
...
def Convolve(input: tensor, weight: tensor):
    ...
    out_h = in_h - weight_h + 1
    out_w = in_w - weight_w + 1

    Output_Tensor = torch.zeros((out_h, out_w))

    for i in range(out_h):
        for j in range(out_w):
            patch = input[i:i+weight_h, j:j+weight_w]
            Output_Tensor[i, j] = torch.sum(patch * weight)
    "*** End Code ***"
...
class DigitConvolutionalModel(Module):
    ...
    def get_loss(self, x, y):
        ...
        return cross_entropy(logits, y.argmax(dim=1))
